<template>
    <div class="m-2">
      ...
      <h1 class="text-black mb-2 text-2xl">Farmacia - Iniciar Venta</h1>
      ...
      >
        ...
        <Column header="Disponible">
          <template #body="slotProps">
            <Tag v-if="slotProps.data.quantity === 0" severity="danger">Agotado</Tag>
      ...
      <DataTable :value="carrito" tableStyle="min-width: 50rem" stripedRows>
        ...
        <template #footer>
          <div class="flex justify-between items-center">
            <span>Total a Pagar: Q. {{ carrito.reduce((sum, item) => sum + item.total, 0).toFixed(2) }}</span>
            <Button
              icon="pi pi-check"
              label="Generar Venta"
              severity="success"
              rounded
              raised
              :disabled="carrito.length === 0 || ventaVariosFarmaciaStatus === 'loading'"
              @click="realizarVenta"
            />
          </div>
        </template>
  ...
  const {
    ...
    mutation: (payload: PayloadSaleMedicineFarmacia[]) =>
      ventaVariosFarmacia(payload),
    onError: (error) => {
      console.error(error);
  ...
  const realizarVenta = () => {
    ...
    const payload = mapLineaVentaMedicineToPayloadSaleMedicineFarmacia(carrito.value);
